import React, { FC, memo, useEffect, useRef, useState } from "react";
import "./_sort.scss";
import { useDispatch, useSelector } from "react-redux";
import {setSort} from '../../redux/filter/filterSlice.ts'
import { RootState } from "../../redux/store.ts";
import { SortPropertyEnum } from "../../redux/filter/types.ts";

const Sort: FC = memo(() => {
  const dispatch = useDispatch()
  const {sort} = useSelector<RootState, {sort:SortItem}>(({filters}) => filters)

  const [open, setOpen] = useState<boolean>(false);

  interface SortItem {
    name: string;
    sortProperty: SortPropertyEnum;
  }

  const list: SortItem[] = [
    { name: "популярності (DESC)", sortProperty: SortPropertyEnum.RATING_DESC },
    { name: "популярності (ASC)", sortProperty: SortPropertyEnum.RATING_ASC },
    { name: "Цені (DESC)", sortProperty: SortPropertyEnum.PRICE_DESC },
    { name: "Цені (ASC)", sortProperty: SortPropertyEnum.PRICE_ASC },
    { name: "Алфавіту (DESC)", sortProperty: SortPropertyEnum.TITLE_DESC },
    { name: "Алфавіту (ASC)", sortProperty: SortPropertyEnum.TITLE_ASC },
  ];

  const sortRef = useRef<HTMLDivElement>(null);

  const handleChangeSort = (obj: SortItem) => {
    dispatch(setSort(obj));
    setOpen(false);
  };

  const handleClickOutside = (e: MouseEvent) => {
    if (sortRef.current && !sortRef.current.contains(e.target as Node)) {
      setOpen(false);
    }
  };

  useEffect(() => {
    document.body.addEventListener("click", handleClickOutside);
    return () => {
      document.body.removeEventListener("click", handleClickOutside);
    };
  }, []);

  return (
    <div ref={sortRef} className="sort">
      <div className="sort__label">
        <svg
          width="10"
          height="6"
          viewBox="0 0 10 6"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z"
            fill="#2C2C2C"
          />
        </svg>
        <b>Сортировка по:</b>
        <span onClick={() => setOpen(!open)}>{sort.name}</span>
      </div>
      {open && (
        <div className="sort__popup">
          <ul>
            {list.map((obj, index) => (
              <li
                className={
                  sort.sortProperty == obj.sortProperty ? "active" : ""
                }
                onClick={() => handleChangeSort(obj)}
                key={index}
              >
                {obj.name}
              </li>
            ))}
          </ul>
        </div>
      )}
    </div>
  );
});

export default Sort;
